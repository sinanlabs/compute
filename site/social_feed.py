...
import os, io, json, datetime as dt, html
...
def main():
    ...
    items = "".join('<item><title>%s</title><link>%s</link><guid isPermaLink="false">%s</guid><pubDate>%s</pubDate><description>%s</description></item>' % (
        html.escape(h["text"][:200]), html.escape(h["url"]), "sinan-social-%s-%s" % (h["date"], h["kind"]), dt.datetime.fromisoformat(h["ts"]).replace(tzinfo=BJ).strftime("%a, %d %b %Y %H:%M:%S %z"), html.escape(h["text"])) for h in reversed(hist))
