...
共识规则与站内一致：同一模型下，两渠道在重叠位置差值恒定视为同一分词器（差值 = 固定前缀）；最大簇 ≥3 渠道才给参考；
...
def main():
    ...
    for m, by in sig.items():
        ...
        if len(c["sites"]) < 3: out[m] = {"ref": None, "peers": len(c["sites"]), "note": "共识样本不足"}; continue
        base = min(c["offs"].values()); ref = [c["ref"].get(i) for i in range(K)]
        out[m] = {"ref": [(x - base) if x is not None else None for x in ref] if False else [ (c["ref"][i] - (c["offs"][c["sites"][0]] - base)) if i in c["ref"] else None for i in range(K)], "peers": len(c["sites"]), "note": None}
