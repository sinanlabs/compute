...
6. **内容指纹**：不跑开源面板的自研聚合站（kie.ai 一类）面板指纹抓不到；对能连上但没有面板签名的域，读少量公开页面，同一页提及 ≥4 家互相竞争厂商的模型族才算"多厂商聚合"并收录。这只是一个可核验的计数，不给"是不是中转站"的标签。

**怎么确认是中转站（一次普通 GET）**：绝大多数中转站跑同一批开源面板（one-api / new-api 一族），有固定形状的状态接口。命中即收录为"已确认"；只返回鉴权错误、没有面板签名的记为"弱指纹"，不进总表，之后再试一次定价接口，拉到报价才升级。我们不做端口扫描、不绕过任何技术措施、每个域名只探一次；厂商官方域与基础设施域（openai.com、claude.ai、cloudflare.com 等）直接排除，不会被当成中转站。
